...
public class ItemHammer extends ItemPickaxe {
   ...
   public boolean onBlockStartBreak(ItemStack stack, int x, int y, int z, EntityPlayer player) {
      MovingObjectPosition raycast = ToolHandler.raytraceFromEntity(player.worldObj, player, true, 10.0D);
      if (raycast != null) {
   ...
   public void breakOtherBlock(EntityPlayer player, ItemStack stack, int x, int y, int z, int originX, int originY, int originZ, int side) {
      ...
      if (!world.isAirBlock(x, y, z)) {
         ...
         ToolHandler.removeBlocksInIteration(player, stack, world, x, y, z, doX ? -range : 0, doY ? -1 : 0, doZ ? -range : 0, doX ? range + 1 : 1, doY ? rangeY * 2 : 1, doZ ? range + 1 : 1, (Block)null, MATERIALS, false, 0, true, false);
      }
   }

   static {
      MATERIALS = new Material[]{Material.rock, Material.iron, Material.ice, Material.glass, Material.piston, Material.anvil, Material.snow, Material.craftedSnow, Material.clay};
   }
}
